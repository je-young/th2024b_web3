...
public class FileUtil {
    ...
    public String fileUpload( MultipartFile multipartFile ) { // (1) 매개변수로 'MultipartFile' 인터페이스 타입을 받는다.
        ...
        File file2 = new File( uploadPath ); if ( !file2.exists() ) { file2.mkdirs(); }
        // (4) File 클래스 : File 관련된 다양한 메소드 제공하는 클래스
        File file = new File( filePath ); // new File( 파일경로 );
        // (5) 경로에 업로드 하기 , transferTo( File 객체 / Path 객체 ) , 예외처리
    ...
    public void fileDownload(String filename , HttpServletResponse response) {
        ...
        try {
            ...
            byte[] bytes = new byte[(int)fileSize]; // 파일의 용량만큼 배열선언 , 배열은 int 타입으로 길이 설정

            // 3. 파일 입력스트림 객체로 파일 읽어도기 , .read( 바이트배열 ) , 읽어온 바이트들을 바이트 배열에 대입
            fin.read(bytes); System.out.println(Arrays.toString(bytes)); // 확인용
            // 4. 파일 입력스트림 닫기 , .close() , 권장 : 스트림에서 이동한 바이트들이 사용한 메모리들을 초기화 하면 안전하다.
            fin.close(); // 입력스트림의 메모리를 직접 닫아준다. 생략시 일정시간 이후 GC 삭제한다.
            ...
            String oldFilename = URLEncoder.encode(filename.split("_")[1] , "UTF-8"); // UUID(사용자에게 보여줄 필용없음)는 제거한다.
            response.setHeader("Content-Disposition", "attachment;filename=" + oldFilename);

            // 1. 서블릿 출력스트림 객체 생성 , response.getOutputStream() : response 객체에서 (바이트)출력스트림 변환
            ServletOutputStream fout = response.getOutputStream();
            // 2. 서블릿 출력스트림 객체 이용한 읽어온바이트 내보내기 , .write( 출력할 배열 )
            fout.write(bytes);
